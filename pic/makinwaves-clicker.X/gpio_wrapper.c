...
// example: "RB12" -> port B, pinpos 12
//static bool GPIO_str2portpos(char *pinStr, char *port, uint8_t *pos)
//{
//    uint8_t pinLen = strlen(pinStr);
//    if (pinLen < 3) return false; // fail
//    *port = pinStr[1];
//    if (*port < 'A' || *port > 'H') return false; // fail
//    *pos = (uint8_t)strtol(&(pinStr[2]), NULL, 10);
//    if (*pos < 1 || *pos > 16) return 0; // fail
//    return true; // success
//}
